...
int CAPECompressCore::EncodeFrame(const void * pInputData, intn nInputBytes)
{
    // variables
    const intn nInputBlocks = nInputBytes / m_wfeInput.nBlockAlign;
    int nSpecialCodes = 0;
    ...
    RETURN_ON_ERROR(Prepare(pInputData, (int) nInputBytes, &nSpecialCodes))
